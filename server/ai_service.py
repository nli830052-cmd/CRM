...
async def analyze_history(history_text: str):
    ...
    2. 모든 대화 날짜를 하나씩 나열하고, 각 날짜별로 무슨 일이 있었는지 1~2문장으로 핵심을 요약하세요.
    3. 다른 부수적인 분석(불만사항, 업무현황 등)은 모두 제외하고 오직 날짜별 타임라인 히스토리에만 집중하세요.
    4. 모든 항목 앞에 불렛 기호(•)를 사용하고 가독성 좋게 줄바꿈하세요.
